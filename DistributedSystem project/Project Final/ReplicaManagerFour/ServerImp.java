...
public class ServerImp {
    ...
    // try {
    // Log(Theater, getFormatDate() + " ManagerClient [" + managerID + "] log in
    // successfully" );
    // } catch (Exception e) {
    // e.printStackTrace();
    // }
    ...
    // try {
    // Log(Theater, getFormatDate() + " UserClient " + studentID + " log in
    // successfully" );
    // } catch (Exception e) {
    // e.printStackTrace();
    // }
            ...
            if ((intQuantity >= 0) && (intmovie == 1 || intmovie == 2 || intmovie == 3)) {
                if (h6.get(movieName).containsKey(movieID)) {
                    ...
                    if (intmovie == 1) {
                        ...
                        h6.put("1", h1);
                        result = "Manager [" + adminID + "] increase quantity of Movie [" + movieID + "] by ["
                                + quantity
                                + "] success";
                    } else if (intmovie == 2) {
                        // newcapacity = h6.get(movieName).get(movieID) + intQuantity;
    ...
    public String bookLocal(String customerID, String movieID, String movieName, String numberOfTickets) {
        ...
        synchronized (this) {
            if (!userTheater.equals(Theater)) {
                ...
                // bookedMovies.entrySet()) {
                if (h8.containsKey(customerID)) {
                    flag = 1;
                    failReason = "User can only book 1 Movie from other theatres";
                }
                // }
    ...
    public String exchangeTickets(String CustomerID, String newMovieID, String oldMovieID, String newmovieName,
            String numberofTickets, String oldmovieName) {
        String result = "";
        String newTheater = newMovieID.substring(0, 3);
        String oldTheater = oldMovieID.substring(0, 3);
        String command = "exchangeTickets(" + CustomerID + "," + newMovieID + "," + oldMovieID + newmovieName + ","
                + numberofTickets + "," + oldmovieName + ")";
        int serverPort;

        if (newTheater.equals(oldTheater)) {
            try {
                if (newTheater.equals(Theater)) {
                    result = exchangeLocal(CustomerID, newMovieID, oldMovieID, newmovieName, numberofTickets,
                            oldmovieName);
    ...
    public String exchangeLocal(String CustomerID, String newMovieID, String oldMovieID, String newmovieName,
            String numberofTickets, String oldmovieName) {
        String result = "";
        if (h8.containsKey(CustomerID) && h8.get(CustomerID).containsKey(oldMovieID)
                && h8.get(CustomerID).get(oldMovieID) >= Integer.parseInt(numberofTickets)) {
            try {
                lock.lock();
